...
namespace DirectKeyDashboard.Views.Charting
{
    ...
    public class QuickConnectOperationTimingsPivotedViewComponent : GroupedBarChartViewComponent {
        ...
        protected async Task<GroupedBarChart> ProjectChart() {
            ...
            // time.
            string rawData = await apiAccess.PullKeyDeviceActivity(DateTime.ParseExact("2019-06-01", "yyyy-MM-dd", CultureInfo.InvariantCulture),
                    DateTime.ParseExact("2020-02-01", "yyyy-MM-dd", CultureInfo.InvariantCulture));
            var serializerSettings = new JsonSerializerSettings();
            serializerSettings.MissingMemberHandling = MissingMemberHandling.Ignore;
            serializerSettings.NullValueHandling = NullValueHandling.Ignore;
            var apiDataModel = JsonConvert.DeserializeObject<ApiDataModel>(rawData, serializerSettings);
            var groups = apiDataModel.Data.Where(d => d.OperationUserIntentDurationMs != 0
                        && d.OperationUserIntentDurationMs.HasValue
                        && d.OperationTotalDurationMs.HasValue
                        && d.OperationUserIntentDurationMs.HasValue)
                    .GroupBy(m => m.OperationCode);
